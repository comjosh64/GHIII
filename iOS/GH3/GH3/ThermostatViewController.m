...
-(NSString*)thermostatID {
   return @"36-6f-92-ff-fe-01-39-e9";
//TODO: Build for each demo device
}
...
//TODO: Build for second demo device
    [request setHTTPBody:[NSJSONSerialization dataWithJSONObject:@{ @"UserName" : @"team17@globalhack.com", @"Password": @"globalhack" } options:0 error:nil]];

    [NSURLConnection sendAsynchronousRequest:request
                                       queue:[NSOperationQueue mainQueue]
                           completionHandler:^(NSURLResponse *response, NSData *data, NSError *error) {
                               if (error) {
                                   NSLog(@"Initial Connection error:%@", [error localizedDescription]);
                               }
...
{
    [self setHeatpointTemperature:@90 units:ThermostatTemperatureUnits_Farenheight];
...
{
//    NSDictionary *jsonDict = [NSJSONSerialization JSONObjectWithData:[[NSData alloc] initWithBase64EncodedString:data options:NSDataBase64DecodingIgnoreUnknownCharacters] options:0 error:nil];
//    NSArray *aArray = jsonDict[@"A"];
//    NSDictionary *opsDict = aArray[1];
